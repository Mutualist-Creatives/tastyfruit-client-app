...
interface MascotDetailProps {
  character: Character;
  prevCharacter?: Character | null;
  nextCharacter?: Character | null;
}

export default function MascotDetail({
  character,
  prevCharacter,
  nextCharacter,
}: MascotDetailProps) {
            ...
            {character.introduction && (
              <p className="text-blue-700 text-xs md:text-lg leading-relaxed">
                {(() => {
                  const nameRegex = new RegExp(character.name, "gi");
                  const parts = character.introduction.split(nameRegex);
                  const matches = character.introduction.match(nameRegex) || [];
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center gap-4 relative">
          ...
          <div className="flex justify-start pointer-events-auto z-10">
            <Link
              href={
                prevCharacter
                  ? `/tasty-universe/${prevCharacter.slug}`
                  : "/tasty-universe"
              }
              className="block"
            >
              ...
              >
                <Image
                  ...
                  alt={prevCharacter ? "Previous mascot" : "Back to universe"}
                  width={20}
                  height={20}
          ...
          <div className="pointer-events-auto absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-0">
            <Link href="/tasty-universe" aria-label="Kembali ke Universe">
              <motion.div
                className="bg-[#B5FE28] rounded-full w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center text-[#003CE2] shadow-md transition-colors"
                whileTap={{ scale: 0.75 }}
                transition={{ type: "spring", stiffness: 400, damping: 15 }}
          ...
          {nextCharacter && (
            <div className="flex justify-end pointer-events-auto z-10">
              <Link
                href={`/tasty-universe/${nextCharacter.slug}`}
                className="block"
              >
                <motion.div
                  className="bg-[#B5FE28] rounded-full w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center text-[#003CE2] shadow-md transition-colors"
